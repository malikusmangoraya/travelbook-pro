...
export async function createOrder(
  user,
  { items, shippingAddress, billingAddress, paymentMethod, notes, promoCode, currency }
) {
  ...
  return sequelize.transaction(async (t) => {
    ...
    const {
      ...
      coupon,
      error: couponError,
    } = await couponService.applyCoupon(promoCode, itemsPrice);
    ...
    const totalPrice =
      Math.round((itemsPrice + taxPrice + shippingPrice - (discount || 0)) * 100) / 100;
