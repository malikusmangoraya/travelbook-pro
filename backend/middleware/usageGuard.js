...
async function _checkCircuitBreaker(identifier) {
  ...
  if (redisWindowCount !== null) {
    if (redisWindowCount > CB_MAX_REQUESTS) {
      ...
      logger.warn(
        `Circuit breaker triggered for ${identifier}: ${redisWindowCount} requests in ${CB_WINDOW_SECS}s`
      );
  ...
  if (fresh.length > CB_MAX_REQUESTS) {
    ...
    logger.warn(
      `Circuit breaker triggered for ${identifier}: ${fresh.length} requests in ${CB_WINDOW_SECS}s`
    );
