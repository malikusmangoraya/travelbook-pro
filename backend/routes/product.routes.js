...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (cached)
      return res.json({ success: true, ...cached, meta: { cached: true, ...(cached.meta || {}) } });
...
router.put('/:id', protect, authorize('admin', 'vendor'), async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && product.vendor_id !== req.user.id) {
      return res
        .status(403)
        .json({ success: false, error: 'Not authorized to update this product' });
    }
...
router.delete('/:id', protect, authorize('admin', 'vendor'), async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && product.vendor_id !== req.user.id) {
      return res
        .status(403)
        .json({ success: false, error: 'Not authorized to delete this product' });
    }
