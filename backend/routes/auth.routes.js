...
router.post('/refresh', authLimiter, async (req, res, next) => {
  try {
    ...
    if (alreadyRevoked) {
      ...
      return res
        .status(401)
        .json({ success: false, error: 'Refresh token has been revoked. Please log in again.' });
    }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!user.twoFactorEnabled) {
        return res
          .status(400)
          .json({ success: false, error: '2FA is not enabled for this account' });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!user.twoFactorSecret) {
        return res
          .status(400)
          .json({ success: false, error: 'No authenticator secret configured. Call setup first.' });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!user.twoFactorEnabled || user.twoFactorMethod !== 'authenticator') {
        return res
          .status(400)
          .json({ success: false, error: 'Authenticator 2FA is not enabled for this account' });
      }

      if (!verifyTOTP(user.twoFactorSecret, code)) {
        return res
          .status(401)
          .json({ success: false, error: 'Invalid or expired authenticator code' });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({ success: false, error: 'Reset link is invalid or has expired' });
      }
...
router.post(
  '/reset-password',
  authLimiter,
  [body('token').notEmpty().withMessage('Reset token is required'), ...resetPasswordValidation],
  async (req, res, next) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
...
router.get('/oauth/google/callback', (req, res, next) => {
  passport.authenticate('google', { session: false }, (err, user) => {
    if (err || !user) {
      return res.redirect(
        `${process.env.FRONTEND_URL || 'http://localhost:5173'}/login?oauth=error`
      );
    }
    const token = generateToken(user.id);
    res.redirect(
