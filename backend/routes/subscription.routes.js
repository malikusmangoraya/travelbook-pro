      ...
      include: [
        {
          model: Plan,
          as: 'plan',
          attributes: ['id', 'name', 'slug', 'amount', 'currency', 'interval', 'features'],
        },
      ],
    });
  ...
  async (req, res, next) => {
    try {
      ...
      if (plan.interval === 'one_time') {
        return res
          .status(400)
          .json({ success: false, error: 'Use /payments/checkout for one-time credit packs' });
      }
      ...
      res
        .status(201)
        .json({ success: true, checkoutUrl: session.url, sessionId: session.id, plan: plan.slug });
    } catch (error) {
      next(error);
    }
  }
