...
router.post('/setup', async (req, res) => {
  try {
    ...
    if (!admin || typeof admin !== 'object')
      return invalid(res, 'Super-Admin details are required');
    ...
    if (!email || !EMAIL_REGEX.test(email))
      return invalid(res, 'Valid admin email is required', 'INVALID_EMAIL');
    if (password.length < 8)
      return invalid(res, 'Password must be at least 8 characters', 'WEAK_PASSWORD');
    ...
    if (!CURRENCY_LIST[currencyCode])
      return invalid(res, `Unsupported currency: ${currencyCode}`, 'INVALID_CURRENCY');
